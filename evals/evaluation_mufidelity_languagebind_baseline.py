...
def parse_args():
    ...
    parser.add_argument('--explanation-method', 
                        type=str, 
                        default='./submodular_results/imagenet-languagebind/slico-0.0-0.05-1.0-1.0/attribution_map',
...
def main(args):
    ...
    for data in tqdm(datas[ : args.eval_number]):
        if os.path.exists(os.path.join(args.explanation_method, data.split(" ")[0].replace(".JPEG", ".npy"))):
            ...
            zoom_factors = (224 / mask.shape[0], 224 / mask.shape[1])
            explanations.append(
                zoom(mask, zoom_factors, order=1)
            )
            # explanations.append(
            #     np.load(
            #         os.path.join(args.explanation_method, data.split(" ")[0].replace(".JPEG", ".npy")))
