...
def parse_args():
    ...
    parser.add_argument('--explanation-method', 
                        type=str, 
                        default='./explanation_results/celeba/Saliency',
                        help='Save path for saliency maps generated by interpretability methods.')
    parser.add_argument('--explanation-smdl', 
                        type=str, 
                        default='./submodular_results-v1-iclr-results/celeba/grad-28x28-8/HsicAttributionMethod-97-1.0-1.0-1.0-1.0/npy',
...
def main(args):
    ...
    # original
    metric = MuFidelity(model, input_image, label_onehot, batch_size=64, nb_samples=32, grid_size=7)

    deletion_score_org = metric(explanations)
    
    batch_mask = convert_smdl_mask(smdl_mask)
    deletion_score = metric(batch_mask)
    print("Original Attribution Method Insertion Score: {}".format(deletion_score_org))
    print("Our Method Insertion Score: {}".format(deletion_score))
    return 
